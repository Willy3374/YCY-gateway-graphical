...
- **画布自由摆放与移动：** 顶层块可拖到画布**任意位置**（像素级，绝对定位），不强制自动排列；
  **已摆放的积木可再次拖动换位置**（pos 被新位置覆盖）。
  **积木吸附：** 拖到某块底部附近（水平重叠且距底缘 14px 内）→ 高亮提示，释放即吸附到其正下方（x 对齐、y 紧贴底缘），并接入它的执行链；拖到无块附近则自由摆放。
  两个事件块可独立摆放，各自开链。JSON 链序按块的位置稳定排序（y 优先、其次 x；无 pos 的按拖入顺序排后），与视觉位置一致。
- **拖入左侧选择栏删除：** 画布积木拖到左侧工具箱上方释放即删除（含下级堆叠一并移除），工具箱高亮「松手删除」提示；从工具箱拖出创建不受影响。
- **数学合理性校验：** 除数非 0；`ln`/`log` 参数 > 0；`asin`/`acos` 参数在 [-1,1]；`tan` 不取 90°+k·180°；`op_random` 下限 ≤ 上限。兼容数字与字符串值（UI 输入框的值是字符串，需转数字再判，否则绕过校验）；只检查字面量参数，嵌套积木的值运行期才知道不在此判。
...
- **线性顺序用 `next`（对象）串联**；`program` 的每一项都是一条以事件积木开头的链。
  **顶层块自由摆放时，链序按块位置稳定排序（y 优先、其次 x）**，与视觉顺序一致；块的位置本身不写入 JSON（`pos` 仅前端内部使用）。
...
- **积木库五分页**：左侧选择栏分为五个分页标签——**事件**（when_start / when_true）、
  ...
  **运算**（数学 / 比较 / 逻辑组合）、**值/变量**（变量 / 当前时间 / 设备状态读取）。
  ...
  空页给出中文提示。分页与校验、生成逻辑解耦，不影响 JSON 契约。

- **拖拽**：从左侧分页积木拖到画布。线性连接用上下顺排；条件 / 运算积木拖到虚线插槽；`✕` 删除；双击库中积木追加到画布。
  **顶层块可拖到画布任意位置，已摆放的可再次拖动换位置**（像素级自由摆放与移动）；
  **拖到某块底部附近即吸附拼接**（对齐正下方并接入执行链，高亮提示）；事件块可独立摆放。
  **拖到左侧任意分页区域仍触发删除**；切换分页本身不触发删除。
...
- **生成 ▶**：校验 → 编译 → 弹命名窗（默认名 = 当前时间 `YYYYMMDDHHmm`）→ 确认后保存并展示 JSON。**校验未通过时不生成 JSON**，输出面板只展示红色错误信息（弹层标题变红、复制/下载按钮禁用），不生成、不保存、不下载。
...
| h | 除数非 0；`ln`/`log` 参数 > 0；`asin`/`acos` ∈ [-1,1]；`tan` ≠ 90°+k·180°；`op_random` 下限 ≤ 上限 | `math_*` | 只查字面量参数 |

画布变化（拖拽删除 / 新增 / 清空）后自动**静默校验**刷新提示；生成 / 开始前**强制校验**，失败即阻止（不生成 JSON，只弹出红色错误信息）。
...
- **踩过的坑（改动时注意）**：
  - 顶层画布的列表容器就是 `#canvas` 本身（`listEl === null`），求插入位置必须回退到根元素，
    否则索引恒为 0、拖进去的顺序会反。
  - 自环检查（不能拖入自身内部）只针对嵌套列表（body/else）：旧判定把目标数组包含自身也视为自环，
    导致顶层已有积木**无法再次拖动换位**（自由移动失效），且测试只测了从工具箱拖入没抓到。
  - 数学合理性校验必须兼容**字符串值**：UI 输入框的值是 `ni.value`（字符串），
    旧判定只查 `typeof === 'number'`，导致 UI 上输入 0 除数依旧能通过校验；引擎编译时才转数字。
