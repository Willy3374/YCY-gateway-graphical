...
- 条件 / 运算 / 值积木**不单独成块**，只作为内联对象嵌入到 `inputs`。
- 未填参数走定义里的 `default`；ID 默认 `1`，强度默认 `0`，时间默认 `1` 秒。
- 空条件槽在编译时自动填充占位条件 `0 = 0`，并在输出面板给出告警——保证产物始终可被解释器执行。
...
- **生成 ▶**：编译 → 弹命名窗（默认名 = 当前时间 `YYYYMMDDHHmm`）→ 确认后保存并展示 JSON。
- **开始**：下发入口。当前为**占位弹窗**，按连接状态提示（未连接到网关 / 正在连接 / 连接异常 / 已连接），真实下发逻辑预留在 `main.js` 的 `openStartDialog()`。
- **校验**：编译并检查结构，报告顶层链是否以事件积木开头、自动填充处数。
